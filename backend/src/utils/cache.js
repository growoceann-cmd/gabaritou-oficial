 ...
 *
 * @example
 * // Busca concursos com cache de 1 hora
...
export function getStats() {
  ...
  for (const [, entry] of store) {
    ...
    try {
      totalBytes += JSON.stringify(entry.value).length * 2; // estimativa UTF-16
    } catch {
      totalBytes += 100; // fallback para valores não serializáveis
    }
  }
